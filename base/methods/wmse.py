...
from methods.whitening import Whitening2dCholesky
from methods.base import BaseMethod
from methods.losses import norm_mse_loss


class WMSE(BaseMethod):
    """ implements BW-MSE loss """
    """ aligned random group """

    def __init__(self, cfg):
        """ init whitening transform """
        super().__init__(cfg)
        self.whitening = Whitening2dCholesky(track_running_stats=False,
                                             eps=cfg.w_eps,
                                             axis=cfg.axis,
                                             minus=cfg.minus,
                                             group=cfg.group)
